...
# FlowDAgger · Agent Collaboration Guide

> Reference implementation of latent-space DAgger for adapting generative robot policies with human-in-the-loop expert corrections.

**Primary toolchain:** Python 3.11 + venv/pip, JAX/CUDA, openpi submodule, and MetaWorld
...
- [[docs/flowdagger-pi05.md]] — flowdagger_pi05 module (`flowdagger_pi05/`)
- [[docs/shared.md]] — shared module (`shared/`)
...
- **`flowdagger_pi05/train_flowdagger.py` (conventionally `dev.sh`)** wraps common development and launch commands: environment setup, lint, format, typecheck, tests, training, inference, evaluation, and data processing. Expose them as subcommands such as `./dev.sh train`, `./dev.sh eval`, and `./dev.sh lint`.
